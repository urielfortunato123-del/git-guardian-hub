...
export function GenLabEnginePage() {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      const resp = await fetch(`${AGENT_URL}/v1/genlab/projects`);
      if (resp.ok) {
  ...
  const handleAnalyze = useCallback(async () => {
    if (!projectId.trim()) {
      toast.error("Informe o ID do projeto");
      return;
    }
    setStep("analyzing");
    setAnalysis(null);
    ...
    try {
      const resp = await fetch(`${AGENT_URL}/v1/genlab/analyze`, {
        method: "POST",
      ...
      setAnalysis(data);
      setStep("idle");
      addLog(`✅ Tipo detectado: ${data.project_type} | Frameworks: ${data.frameworks.join(", ")}`);
      toast.success(`Projeto classificado: ${data.project_type}`);
    } catch (e: any) {
      setStep("error");
      addLog(`❌ Erro: ${e.message}`);
      toast.error("Falha na análise. Verifique se o agent está rodando.");
    }
  }, [projectId, addLog]);
  ...
  const handleRecreate = useCallback(async () => {
    ...
    setStep("recreating");
    addLog(`🧬 Recriando projeto: ${projectId}`);
    try {
      const resp = await fetch(`${AGENT_URL}/v1/genlab/recreate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ project_id: projectId }),
      });
      ...
      if (data.ok) {
        setStep("done");
        addLog(`✅ Projeto recriado: ${data.count} arquivos gerados`);
        toast.success(`Projeto recriado com sucesso! ${data.count} arquivos`);
        fetchProjects();
      } else {
      ...
      setStep("error");
      addLog(`❌ Erro: ${e.message}`);
      toast.error("Falha na recriação. Verifique IA local.");
    }
  }, [projectId, addLog, fetchProjects]);

  const handleRun = useCallback(async (name: string) => {
    setRunningProject(name);
    addLog(`▶️ Executando projeto: ${name}`);
    try {
      const resp = await fetch(`${AGENT_URL}/v1/genlab/run`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await resp.json();
      if (data.ok) {
        addLog(`✅ Projeto rodando (${data.mode})`);
        toast.success(`Projeto iniciado no modo ${data.mode}`);
      } else {
        addLog(`❌ Falha: ${data.error}`);
        toast.error(data.error || "Falha ao executar");
      }
    } catch (e: any) {
      addLog(`❌ ${e.message}`);
      toast.error("Erro ao executar projeto");
    } finally {
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4">
        ...
        {step === "error" && (
          <div className="flex items-center gap-2 text-sm text-destructive">
            ...
            Ocorreu um erro. Verifique os logs abaixo.
          </div>
        )}
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-4">
        ...
        {generatedProjects.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">
            Nenhum projeto gerado ainda. Use o pipeline acima para recriar projetos.
          </p>
        ) : (
              ...
              <div key={proj.name} className="flex items-center justify-between p-3 bg-secondary/50 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-accent text-accent-foreground rounded-md text-xs font-medium hover:opacity-90 disabled:opacity-50"
                  >
                    {runningProject === proj.name ? <Loader2 className="w-3 h-3 animate-spin" /> : <Play className="w-3 h-3" />}
                    Rodar
                  </button>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary text-foreground rounded-md text-xs font-medium hover:bg-secondary/80">
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input defaultValue="gemma3" className="w-full bg-secondary border border-border rounded-lg px-3 py-2 text-sm text-foreground" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Base URL</label>
            <input defaultValue="http://127.0.0.1:11434/v1" className="w-full bg-secondary border border-border rounded-lg px-3 py-2 text-sm text-foreground" />
          </div>
        </div>
      </div>
